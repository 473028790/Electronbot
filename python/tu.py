...
def convert_to_18bit_true_color(frame):
    # 将帧转换为PIL图像格式，并将其转换为RGB格式
...
def export_to_c_array(byte_array, output_path, frame_number):
    # 将 byte_array 拆分为 4 个部分
    quarter_size = len(byte_array) // 4
    parts = [byte_array[i * quarter_size:(i + 1) * quarter_size] for i in range(4)]

    # 创建C数组的字符串表示
    c_array_str = f"const uint8_t gImage_frame_{frame_number}[4][{quarter_size}] = {{\n"
...
# 视频路径
video_path = r'C:\Users\47302\Desktop\Emoji\right.mp4'
# 输出路径
output_path = r'C:\Users\47302\Desktop\gImage_frames.txt'
# 串口配置
port = 'COM30'
...
while cap.isOpened():
    ...
    if frame_number % 4 == 0:
        ...
        # 导出为C数组格式的txt文件
        export_to_c_array(byte_array, output_path, frame_number)
