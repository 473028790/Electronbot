...
void CAN1_Filterinit_And_Start(void)
{
	...
	HAL_CAN_ConfigFilter(&hcan1,&can1_filter);//闁跨喐鏋婚幏宄邦潗闁跨喐鏋婚�?? CAN1 闁跨喐鏋婚幏鐑芥晸閺傘�?��?�归柨鐔告灮�???
	HAL_CAN_Start(&hcan1);//闁跨喐鏋婚幏鐑芥晸閺傘�?��??? CAN1
	HAL_CAN_ActivateNotification(&hcan1,CAN_IT_RX_FIFO0_MSG_PENDING);//闁跨喐鏋婚幏鐑芥晸閺傘�?��?�笴AN1 FIFO0 闁跨喐鏋婚幏鐑芥晸閺傘�?��???
}
...
void Can_Get_uint8(uint8_t motor_id,uint8_t type,uint8_t data)
{
	CAN_TxHeaderTypeDef tx;
	uint32_t TxMailbox;
	tx.StdId = 0x258+motor_id;
	tx.RTR = CAN_RTR_DATA;
	tx.IDE = CAN_ID_STD;
	tx.DLC = 8;
	...
	Data[7]=0;
	//HAL_CAN_AddTxMessage(&hcan1,&tx,Data,(uint32_t *)CAN_TX_MAILBOX0);
	HAL_CAN_AddTxMessage(&hcan1, &tx, Data, &TxMailbox);
}
