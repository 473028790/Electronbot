...
static int8_t CDC_Init_HS(void)
{
  ...
  USBD_CDC_SetTxBuffer(&hUsbDeviceHS, usbBuffer.extraDataTx, 0);
  USBD_CDC_SetRxBuffer(&hUsbDeviceHS, usbBuffer.rxData[usbBuffer.pingPongIndex]);
  return (USBD_OK);
  /* USER CODE END 8 */
}
...
static int8_t CDC_Receive_HS(uint8_t* Buf, uint32_t *Len)
{
  ...
  // Append the data if we received 512 bytes
  if (usbBuffer.receivedPacketLen == 512)
  {
      usbBuffer.rxDataOffset += usbBuffer.receivedPacketLen;
  }
  // If we received exactly 224 bytes, switch PingPongBuffer
  if (usbBuffer.receivedPacketLen == 224)
  {
      SwitchPingPongBuffer();
  }
  // Get the pointer to the PingPong buffer
